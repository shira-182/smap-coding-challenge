...
class SummaryView(ListView, LoggingMixin):
    ...
    def render_to_response(self, context, **response_kwargs):
        ...
        self.logger.info(f"処理にかかった時間: {elapsed_time}")
        return super().render_to_response(context, **response_kwargs)
    ...
    def _create_df_monthly(self):
        df = pd.DataFrame(
            list(CalculatedConsumption.objects.all().values("date", "sum"))
        )
    ...
    def _create_df_daily(self):
        ...
        df = pd.DataFrame(
            list(
                CalculatedConsumption.objects.filter(date__gte=december).values(
                    "date", "sum"
                )
            )
        )
...
class SummaryDetail(DetailView, LoggingMixin):
    ...
    def render_to_response(self, context, **response_kwargs):
        ...
        ax2 = df_daily.plot(ax=axes[1])
        ax1.set_title(
            f"月ごとの消費量(area:{user.area} tariff:{user.tariff})", fontname=FONTNAME
        )
    ...
    def _create_df_monthly(self, user):
        df = pd.DataFrame(
            list(
                Consumption.objects.filter(user=user).values("datetime", "consumption")
            )
        )
    ...
    def _create_df_daily(self, user):
        ...
        df = pd.DataFrame(
            list(
                Consumption.objects.filter(user=user, datetime__gte=december).values(
